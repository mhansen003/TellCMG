  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={onToggle}
          className="fixed left-4 top-1/2 -translate-y-1/2 z-40 p-3 rounded-xl bg-bg-card border-2 border-border-subtle hover:border-cmg-blue/50 text-text-secondary hover:text-cmg-blue transition-all shadow-lg group"
        ...
        >
          ...
          {history.length > 0 && (
            <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-cmg-blue text-white text-xs font-bold flex items-center justify-center">
      ...
      >
        ...
        <div className="relative h-full w-80 bg-bg-secondary border-r-2 border-border-subtle shadow-2xl flex flex-col">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-4 border-b border-border-subtle bg-gradient-to-r from-cmg-blue/10 to-transparent">
            <div className="flex items-center gap-2">
              <svg className="w-5 h-5 text-cmg-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <h2 className="text-base font-bold text-text-primary">Idea History</h2>
              <span className="px-2 py-0.5 rounded-full bg-cmg-blue/20 text-cmg-blue text-xs font-semibold">
                {history.length}
              </span>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {history.length === 0 ? (
              <div className="text-center py-12 text-text-muted">
                ...
                <p className="text-xs mt-1">Structured ideas will appear here</p>
              </div>
            ) : (
              history.map((item) => (
                <div
                  ...
                  className={`
                    ...
                    ${
                      activeId === item.id
                        ? "bg-cmg-glow border-cmg-blue"
                        : "bg-bg-card border-border-subtle hover:border-cmg-blue/30"
                    }
                  `}
                >
                  {/* Mode badge */}
                  <div className="flex items-center justify-between mb-2">
                    <span className={`
                      ...
                      ${activeId === item.id ? "bg-cmg-blue text-white" : "bg-bg-elevated text-text-secondary"}
                    `}>
                      {item.mode}
                    </span>
                  ...
                  {/* Active indicator */}
                  {activeId === item.id && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-cmg-blue rounded-r-full" />
                  )}
                </div>
              ))
            )}
