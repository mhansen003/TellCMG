  ...
  return (
    <>
      ...
      {isVisible && (
        ...
        >
          <div className="relative px-3 py-2 rounded-lg bg-bg-elevated border border-cmg-blue/30 shadow-xl shadow-black/20 max-w-xs">
            {/* Arrow */}
            <div
              className={`absolute w-2 h-2 bg-bg-elevated border-cmg-blue/30 transform rotate-45 ${
                position === "top"
...
export function TooltipIcon({ content, position = "top" }: TooltipIconProps) {
  return (
    <Tooltip content={content} position={position}>
      <button
        type="button"
        className="w-4 h-4 rounded-full bg-bg-elevated border border-border-subtle flex items-center justify-center text-text-muted hover:text-cmg-blue hover:border-cmg-blue/50 transition-colors cursor-help"
