      ...
      {!isExpanded && selected.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          {selectedModifiers.map((modifier) => (
            <span
              key={modifier.id}
              className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-cmg-blue/10 text-cmg-blue text-xs font-medium"
            >
              {modifier.label}
              <button
                onClick={(e) => { e.stopPropagation(); toggleModifier(modifier.id); }}
            ...
            return (
              <label
                ...
                className={`
                  ...
                  ${
                    isChecked
                      ? "bg-cmg-glow border-cmg-blue/50"
                      : "bg-bg-elevated/50 border-border-subtle hover:border-cmg-blue/30"
                  }
                `}
              >
                <input
                  ...
                  className="checkbox-cmg flex-shrink-0"
                />
                <span
                  className={`text-xs font-medium truncate ${isChecked ? "text-cmg-blue" : "text-text-secondary"}`}
