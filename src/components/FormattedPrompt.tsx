...
export default function FormattedPrompt({ content }: FormattedPromptProps) {
  // Custom theme based on VS Code Dark+ with CMG blue accents
  const customTheme = {
    ...vscDarkPlus,
    'pre[class*="language-"]': {
      ...
      elements.push(
        <div key={`code-${blockIndex}`} className="my-3 rounded-xl overflow-hidden border border-border-subtle">
          ...
          <div className="bg-bg-elevated px-3 py-1.5 border-b border-border-subtle flex items-center gap-2">
            <span className="text-xs font-mono text-cmg-blue font-semibold uppercase">{language}</span>
            <div className="flex gap-1 ml-auto">
              <span className="w-2.5 h-2.5 rounded-full bg-accent-rose/60"></span>
              <span className="w-2.5 h-2.5 rounded-full bg-cmg-gold/60"></span>
  ...
  const renderTextContent = (text: string, keyPrefix: number) => {
          ...
          <ul key={`${keyPrefix}-list-${listKey++}`} className="list-none space-y-1 my-2 ml-4">
            {currentList.map((item, i) => (
              <li key={i} className="flex items-start gap-2 text-text-secondary">
                <span className="text-cmg-blue mt-0.5">•</span>
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmedLine.startsWith("## ")) {
        ...
        elements.push(
          <h3 key={`${keyPrefix}-${index}`} className="text-sm font-bold text-cmg-blue mt-4 mb-2 flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-cmg-blue" />
            {headingText}
          </h3>
        );
        return;
        ...
        if (match) {
          elements.push(
            <div key={`${keyPrefix}-${index}`} className="flex items-start gap-2 my-1 ml-2">
              <span className="flex-shrink-0 w-5 h-5 rounded-full bg-cmg-blue/20 text-cmg-blue text-xs font-bold flex items-center justify-center">
                {match[1]}
              </span>
